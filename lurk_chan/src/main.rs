use std::{path::PathBuf, fs::{create_dir_all, File}, io::BufReader, collections::HashMap, time::Duration};
use std::sync::Arc;
mod commands;
use anyhow::{Context as _, Error, anyhow, bail};
use async_shutdown::ShutdownManager;
use common::{Report, Action, Location};
use lurk_chan::{transmute_json, update_report_message, update_audit_message};
use poise::{BoxFuture, Framework, FrameworkOptions, serenity_prelude::{EventHandler, async_trait, FullEvent, ActivityData, self, CreateMessage, ComponentInteraction, CreateInteractionResponse, CreateInteractionResponseFollowup, EditInteractionResponse, CreateInteractionResponseMessage, Timestamp}, FrameworkContext, execute_modal, execute_modal_on_component_interaction, Modal};
...
async fn handle(evt: &FullEvent, framework: FrameworkContext<'_,LurkChan, Error>) -> anyhow::Result<()> {
    match evt {
        FullEvent::Ready { ctx, data_about_bot} => {
            info!("And {} v{} takes the stage!", data_about_bot.user.name, env!("CARGO_PKG_VERSION"));
        ...
        FullEvent::InteractionCreate { ctx, interaction } => {
            if let Some(r) = interaction.as_message_component() {
                if let Err(e) = on_button(ctx, r, framework.user_data).await {
                    ...
                    r.create_response(ctx, 
                        CreateInteractionResponse::Message(
                            CreateInteractionResponseMessage::default().content("Error handling button. yell at wackery please")
...
pub async fn on_button(ctx: &serenity_prelude::Context, int: &ComponentInteraction, lc: &LurkChan) -> anyhow::Result<()> { 
    // piss shit anmd die
    ...
    match kind {
        ...
        "close" => {
            let report = lc.db.get_report_from_id(id).await?.context("That report dont exist")?;
            if report.claimant.is_some_and(|i| i == uid) {
                // fuck
                let resp = execute_modal_on_component_interaction(WhatTheFuck(ctx), Arc::new(int.clone()), Some(AuditModal {
                ...
                let m = channel_for_msg.send_message(ctx, CreateMessage::default().embed(
                    lurk_chan::create_action_embed(&a, ctx, aid, channel_for_msg).await?
                ).components(lurk_chan::create_action_components(aid))).await?;
                lc.db.add_action_message(m.channel_id.get(), m.id.get(), aid).await?;
                update_report_message(ctx, id, &lc.db).await?;
                int.create_followup(
                    ctx,
        ...
        "edit" => {
            let action = lc.db.get_action_from_id(id).await?.context("That action do no exis")?;
            if action.claimant != uid {
                int.create_response(
                    ctx,
                    CreateInteractionResponse::Message(CreateInteractionResponseMessage::default()
                        .content("sorry buddy, that doesn't belong to you")
                        .ephemeral(true),
                )).await?;
                return Ok(());
            ...
            let resp = execute_modal_on_component_interaction(WhatTheFuck(ctx), Arc::new(int.clone()), Some(Into::<AuditModal>::into(action.clone())), Some(Duration::from_secs(120))).await?;
            if resp.is_none() {
                return Ok(())
            }
            let resp = resp.unwrap();
            ...
            lc.db.edit_action(id, a, Timestamp::now().to_string(), uid).await?;
    ...
    int.edit_response(ctx, EditInteractionResponse::default().content(":+1:")).await?;
    Ok(())
}
...
async fn on_message(ctx: &serenity_prelude::Context, new_message: &serenity_prelude::Message, lc: &LurkChan) -> anyhow::Result<()> {
    ...
    if let Some(report) = report_from_msg(&new_message)? {
        ...
        let m = new_message.channel_id.send_message(ctx, CreateMessage::default()
            .embed(embed)
            .components(comp)
        ).await?;
        lc.db.add_report_message(m.channel_id.get(), m.id.get(), id).await?;
...
async fn bot(config: Config, db: Database, s: ShutdownManager<&'static str>) -> anyhow::Result<()> {
    ...
    let client = Client::builder(&config.main.token, GatewayIntents::non_privileged() | GatewayIntents::MESSAGE_CONTENT)
        .framework(Framework::new(FrameworkOptions {
            commands: commands::commands(),
            event_handler: |evt, ctx, _| Box::pin(async move {
            ...
            Box::pin(async move { 
                ...
                poise::builtins::register_in_guild(ctx, &framework.options().commands, config.discord.debug_guild.context("No debug guild in debug mode!")?).await?;
                #[cfg(not(debug_assertions))]
                poise::builtins::register_globally(ctx, &framework.options().commands).await.context("failed to register commands")?;
                Ok(LurkChan {
                    config,
                    db,
    ...
    tokio::task::spawn(client_wait_shutdowner.wrap_delay_shutdown(async move {
