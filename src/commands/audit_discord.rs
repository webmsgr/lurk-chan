use std::error::Error;
use serenity::all::{CommandInteraction, CommandOptionType, CommandType, CreateInteractionResponseFollowup};
...
pub async fn run(ctx: &Context, interaction: &CommandInteraction) -> Result<(), Box<dyn Error + Send + Sync>> {
    let (user, u) = interaction.data.resolved.users.iter().next().expect("Failed to get user");
    ...
    interaction.create_response(ctx, audit_log_modal(None, Some(Report {
        reported_id: user.get().to_string(),
        reported_name: nick,
        ..Default::default()
    }), Location::Discord)).await?;
...
pub mod slash {
    use super::*;
    pub fn register() -> CreateCommand {
        CreateCommand::new("discord").description("Create a new audit log entry for a discord user").add_option(
            CreateCommandOption::new(CommandOptionType::User, "user", "User to audit").required(true)
        )
    }
}