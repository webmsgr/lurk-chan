...
use serenity::all::{
    CommandInteraction, CommandOptionType, CreateCommandOption, CreateEmbed, Timestamp,
};
...
pub async fn run(ctx: &Context, interaction: &CommandInteraction) -> anyhow::Result<()> {
    ...
    let whr = match interaction
        .data
        .options
        .get(0)
        .context("No option found for where")?
        .value
        .as_str()
        .context("not a fucking string")?
    {
    ...
    let whatthefuck = interaction
        ...
        .value
        .as_str()
        .context("whatthefuck is not a str")?;
    ...
    let q = match whr {
        WhereTheFuck::Actions => {
            let q: Vec<(i64, Action)> =
                query_file!("src/data/query/search_actions.sql", whatthefuck)
                    .fetch_all(&mut d)
                    .await?
                    .into_iter()
                    .map(|i| {
            ...
            let action_count = sqlx::query!(
                "select count(*) as \"count\" from ActionsSearch where ActionsSearch match ?;",
                whatthefuck
            )
            .fetch_one(&mut d)
            .await?
        ...
        WhereTheFuck::Reports => {
            let q: Vec<(i64, Report)> =
                query_file!("src/data/query/search_reports.sql", whatthefuck)
                    .fetch_all(&mut d)
                    .await?
                    .into_iter()
                    .map(|i| {
                        (
                            ...
                            Report {
                                ...
                                report_reason: i
                                    .report_reason
                                    .expect("Fuck you, this should never fail"),
    ...
    let embed = match q {
        Resultz::Actions((res, c)) => e
            .description(res.into_iter().fold(String::new(), |mut o, a| {
                let _ = writeln!(o, "* {} - `{}` -> `{}`\n", a.0, a.1.offense, a.1.action);
                o
            }))
            .footer(CreateEmbedFooter::new(format!("{} results total", c))),
        Resultz::Reports((res, c)) => e
            .description(res.into_iter().fold(String::new(), |mut o, a| {
    ...
    interaction
        .edit_response(&ctx, EditInteractionResponse::new().embeds(vec![embed]))
        .await?;
    Ok(())
}
