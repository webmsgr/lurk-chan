...
async fn on_model(
    ...
    let id: i64 = c
        .collect::<String>()
        .parse()
        .expect("Custom id to be a number");
    let u = &modl.user;
...
async fn on_interaction_button(
    ...
    match kind {
        ...
        "close" => {
            ...
            int.create_response(
                &ctx,
                audit_log_modal(Some(id), 'r', report, Location::SL, None),
            )
            .await?;
            return Ok(());
            /*if let Err(e) = query!(
        ...
        "edit" => {
            ...
            if let Some(a) = action {
                ...
                int.create_response(
                    &ctx,
                    audit_log_modal(Some(id), 'a', r, a.server, Some(a.action)),
                )
                .await?;
                return Ok(());
            }
